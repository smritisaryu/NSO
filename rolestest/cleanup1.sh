...
if [ -n "$subnets" ]; then
    echo "subnets found: $subnet_name"
else
    echo "No subnets found"
fi

router_id=$(openstack router show "$router_name" -f value -c id 2>/dev/null)
if [ -n "$router_id" ]; then
    for subnet_id in $(openstack router show "$router_id" -f value -c interfaces_info 2>/dev/null | grep -oP 'subnet_id=\K\S+'); do
        echo "Removing subnet $subnet_id from router $router_name"
...
ports=$(openstack port list --format "value" --column "ID")

count=$(echo "$ports" | wc -l)  # Count the number of ports
